...
class DataFlow(object):
        ...
        if (
            ...
            if missing_data_names:
                self.print_progress(
                    3,
                    '  --> {} requires {}'.format(
                        data_name, ', '.join(missing_data_names)
                    ),
                )
    ...
    def get_data_lite_and_broad(self, data_name, **kwargs):
        # determine what the data part of kwargs is
        input_data = {
            k: v for k, v in kwargs.items() if k in self.data_dependencies[data_name]
        }
        ...
        if missing_data_names:
            for missing_dependency in missing_data_names:
                input_data[missing_dependency] = self.get_data_lite_and_broad(
                    data_name=missing_dependency, **kwargs
                )
        # make the data
        if data_name in list(self.data_makers.keys()):
